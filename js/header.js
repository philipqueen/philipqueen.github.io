document.addEventListener('DOMContentLoaded', () => {
    const version = '1.0.2';
    const cachedVersion = localStorage.getItem('pqHeaderVersion');
    const cachedHeader = localStorage.getItem('pqHeaderHTML');
    ...
    if (cachedHeader && cachedVersion === version) {
        ...
        fetch('/header.html?v=${version}')
            ...
            .then(data => {
                ...
                localStorage.setItem('pqHeaderHTML', data);
                localStorage.setItem('pqHeaderVersion', version);
                initNavMenu();
            });
    }
    setupToggle();
